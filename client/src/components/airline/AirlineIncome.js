...
import { showAirlineIncome } from "../../store/airline/actions";

export default function AirlineIncome() {
  const [startDate, setStartDate] = useState();
  const [endDate, setEndDate] = useState();
  const classes = useStyles();
  const airline = useSelector(selectAirlineDetails);
  const dispatch = useDispatch();
  const income = useSelector(selectIncomeData);
  function handleShowButon() {
    dispatch(
      showAirlineIncome({
        id: airline.id,
        startDate: startDate,
