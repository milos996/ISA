...
export function* fetchAirlineDetails(){
    ...
    const { data } = yield call(airlineService.fetchAirlineDetails, payload.airlineId);
    yield put(putAirlineDetails(data));
}

export function* fetchDestinations(){
    const { payload } = yield take(FETCH_AIRLINE_DESTINATIONS);
    const { data } = yield call(flightService.fetchDestinations, payload.airlineId);
...
export function* fetchTicketsForFastReservation(){
    ...
    const { data } = yield call(flightService.fetchTicketsForFastReservation, payload.airlineId);
...
export function* createFastTicketReservation(){
    ...
    yield call(airplaneTicketService.createFastTicketReservation, payload.flightId);
}
...
export function* fetchAirplanes(){
    ...
    const { data } = yield call(airplaneService.fetchAirplanes, payload.airlineId); 
